...
const LeadDetails = () => {
    ...
    return (
      <div className="flex-grow flex items-center justify-center bg-gray-50 h-full">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs font-semibold text-gray-500">Loading Lead Profile...</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !lead) {
    return (
      <div className="flex-grow p-8 bg-gray-50 h-full">
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center max-w-md mx-auto shadow-sm mt-12">
          <h3 className="font-bold text-gray-700 text-lg">Lead Profile Missing</h3>
          <p className="text-xs text-gray-400 mt-2 mb-6">{error || 'Record was not found.'}</p>
  ...
  return (
    <div className="flex-1 p-8 bg-gray-50 overflow-y-auto h-full fade-in">
      ...
      <div className="mb-6">
        <Link to="/leads" className="inline-flex items-center gap-1.5 text-xs font-bold text-gray-500 hover:text-blue-600 transition">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <div className="flex items-center gap-4 border-b border-gray-100 pb-5 mb-5">
              ...
              <div className="min-w-0">
                <h2 className="text-lg font-bold text-gray-800 truncate">{lead.name}</h2>
                <p className="text-xs text-gray-500 font-semibold truncate">{lead.company || 'Individual Lead'}</p>
              </div>
            </div>
            ...
            <div className="space-y-4 text-xs font-medium text-gray-600">
              {/* Status Selector */}
              <div>
                <label className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-1">Pipeline Status</label>
                <select
                  value={lead.status}
                  onChange={handleStatusChange}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs font-bold bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition"
                >
              ...
              {/* Phone */}
              <div>
                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-0.5">Phone Number</span>
                <span className="text-gray-800 font-semibold block">{lead.phone || '-'}</span>
              </div>

              {/* Lead Source */}
              <div>
                <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-0.5">Acquisition Source</span>
                <span className="text-gray-800 font-semibold block">{lead.source}</span>
              </div>

              {/* Dates */}
              <div className="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4">
                <div>
                  <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-0.5">Last Contact</span>
                  <span className="text-gray-800 font-semibold block text-[11px]">
                    {lead.lastContacted 
                ...
                <div>
                  <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block mb-0.5">Follow-Up Target</span>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 flex flex-col h-full">
            <div className="border-b border-gray-100 pb-4 mb-4">
              <h3 className="font-bold text-gray-800">Discussion Logs & Remarks</h3>
              <p className="text-xs text-gray-400">Keep a detailed timeline of client follow-ups and interactions.</p>
            </div>
            ...
            <form onSubmit={handleAddNote} className="mb-6">
              <textarea
                ...
                rows={3}
                className="w-full border border-gray-200 rounded-lg p-3 text-xs font-semibold text-gray-700 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-150"
              ></textarea>
              <div className="flex justify-end mt-2">
                <button
                  type="submit"
                  disabled={noteSubmitting || !noteContent.trim()}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-200 text-white disabled:text-gray-400 rounded-lg text-xs font-bold shadow-sm transition"
                >
            ...
            <div className="space-y-4">
              {lead.notes.length === 0 ? (
                <div className="py-12 border-2 border-dashed border-gray-100 rounded-xl flex flex-col items-center justify-center text-center px-4">
                ...
                <div className="relative border-l-2 border-gray-100 pl-6 ml-3 space-y-6">
                  ...
                  {[...lead.notes].reverse().map((note) => (
                    <div key={note._id} className="relative group fade-in">
                      ...
                      <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 shadow-sm relative group-hover:border-gray-200 transition">
                        <div className="flex justify-between items-start gap-4">
                          <p className="text-xs font-semibold text-gray-700 leading-relaxed whitespace-pre-wrap">{note.content}</p>
                          <button
                            onClick={() => handleDeleteNote(note._id)}
                            className="text-gray-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition duration-150"
                        ...
                        <div className="text-[10px] text-gray-400 font-bold mt-2.5 flex items-center gap-1">
